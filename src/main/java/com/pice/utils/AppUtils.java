...
public final class AppUtils {
    ...
    private static String getAppBundleId() {
        return com.pice.config.ConfigManager.get("app.bundle.id", "com.your.app.package");
    }

    private static String cachedDeviceSerial = null;

    /**
     * Get the serial number of the connected Android device dynamically.
     */
    public static String getConnectedDeviceSerial() {
        if (cachedDeviceSerial != null) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.endsWith("device") && !line.startsWith("List")) {
                    cachedDeviceSerial = line.split("\\s+")[0];
                    return cachedDeviceSerial;
                }
            }
        } catch (Exception e) {
            log.debug("Error getting device serial via adb devices: {}", e.getMessage());
        }
        cachedDeviceSerial = "10MG56FM6E000FD"; // fallback default
        return cachedDeviceSerial;
    }

    /**
     * Set developer options on the connected Android device dynamically.
     */
    public static void setDeveloperOptions(boolean enabled) {
        String serial = getConnectedDeviceSerial();
        if (!enabled && !serial.startsWith("emulator-")) {
            log.info("Physical device detected — skipping disabling developer options to avoid ADB disconnection");
            return;
        }
        String val = enabled ? "1" : "0";
        log.info("Setting developer options to {} on device {}", val, serial);
        AppiumDriver driver = DriverManager.getDriver();
        if (driver instanceof AndroidDriver androidDriver) {
            try {
                Map<String, Object> shellArgs = new HashMap<>();
                shellArgs.put("command", "settings");
                shellArgs.put("args", java.util.List.of("put", "global", "development_settings_enabled", val));
                androidDriver.executeScript("mobile: shell", shellArgs);
                log.info("Developer options set to {} via mobile:shell", val);
                return;
            } catch (Exception e) {
                log.debug("mobile:shell settings failed: {}. Falling back to Runtime.exec", e.getMessage());
            }
        }
        ...
        try {
            ...
            if (androidHome == null) androidHome = System.getProperty("user.home") + "/Library/Android/sdk";
            String adb = androidHome + "/platform-tools/adb";
            Process proc = Runtime.getRuntime().exec(new String[]{adb, "-s", serial, "shell", "settings", "put", "global", "development_settings_enabled", val});
            proc.waitFor(3, java.util.concurrent.TimeUnit.SECONDS);
            log.info("Developer options set to {} via Runtime.exec adb", val);
        } catch (Exception ex) {
            log.error("Failed to set developer options: {}", ex.getMessage());
        }
    }
}
