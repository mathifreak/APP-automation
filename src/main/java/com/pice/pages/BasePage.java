...
import com.pice.exceptions.ElementInteractionException;
import com.pice.exceptions.PageNotLoadedException;
...
public abstract class BasePage {
    ...
    /**
     * Tap on the top-left quadrant of an element (useful to bypass bottom-right diagonal overlays).
     *
     * @param locator the By locator
     */
    protected void tapTopLeft(By locator) {
        log.debug("Tapping top-left of element: {}", locator);
        ...
        int tapX = location.getX() + size.getWidth() / 4;
        int tapY = location.getY() + size.getHeight() / 4;
        com.pice.utils.GestureUtils.tapAt(tapX, tapY);
    }
